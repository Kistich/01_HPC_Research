...
class UserUsageAnalyzer:
    """User CPU Usage Analyzer"""
    ...
    def compare_monthly_patterns(self, output_dir='figures'):
        """Compare monthly user usage patterns across all groups"""
        ...
        plt.title('Monthly Average User CPU Usage by Server Type', fontsize=18)
        plt.xlabel('Month', fontsize=14)
        plt.ylabel('Average User CPU Usage (%)', fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/all_groups_monthly_user_usage.png', dpi=300)
        plt.close()
        
        return monthly_avgs
    
    def compare_weekly_patterns(self, output_dir='figures'):
        """Compare weekly user usage patterns across all groups"""
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        ...
        plt.title('Weekly Average User CPU Usage by Server Type', fontsize=18)
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/all_groups_weekly_user_usage.png', dpi=300)
        plt.close()
        
        return weekly_avgs

    def analyze_daily_patterns(self, output_dir='figures'):
        """Analyze daily user usage patterns for all server groups"""
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        ...
        for group_name, group_data in all_data.items():
            ...
            plt.title(f'Daily User CPU Usage Pattern for {group_name}', fontsize=18)
    ...
    def find_high_usage_periods(self, threshold=80, duration_hours=2, output_dir='figures'):
        """Find periods of sustained high user usage"""
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        ...
        for group_name, group_data in all_data.items():
            ...
            if group_results:
                ...
                if server_names:  # Check if there's data to plot
                    ...
                    plt.title(f'High User CPU Usage Periods for {group_name} (>{threshold}%)', fontsize=18)
    ...
    def generate_heatmap(self, output_dir='figures'):
        """Generate heatmap showing user usage patterns"""
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        ...
        for group_name, group_data in all_data.items():
            ...
            ax = sns.heatmap(
                ...
                fmt=".1f",
                linewidths=0.5,
                cbar_kws={'label': 'Average User CPU Usage (%)'}
            )
            ...
            plt.title(f'User CPU Usage Heatmap for {group_name}', fontsize=18)
            plt.xlabel('Hour of Day', fontsize=14)
    ...
    def compare_all_groups(self, output_dir='figures'):
        ...
        # Create bar chart for average user usage
        plt.figure(figsize=(18, 10))
        ...
        plt.title('Average User CPU Usage Comparison Across Server Groups', fontsize=18)
        ...
        stats_df.to_csv(f'{output_dir}/user_usage_statistics.csv')
...
def main():
    ...
    excel_file = hpc_research_dir / "Stage00_HPC_raw_data" / "prometheus_metrics_data_CPU使用率_用户使用率（%）_20250221_110911.xlsx"
    ...
    # Initialize analyzer
    analyzer = UserUsageAnalyzer(str(excel_file))
